...
<head>
    ...
    <title><?= htmlspecialchars($category['tenLoaiSP'] ?? 'Danh mục sản phẩm') ?> - ToyShop</title>
    <style>
        :root {
            --primary-red: #e31837;
            --primary-blue: #003399;
...
<div class="breadcrumb">
    <div class="container">
        ...
        <span> / </span>
        <span><?= htmlspecialchars($category['tenLoaiSP'] ?? 'Danh mục') ?></span>
    </div>
</div>
...
<div class="page-header">
    <div class="container">
        ...
        <h1><?= htmlspecialchars($category['tenLoaiSP'] ?? 'Danh mục sản phẩm') ?></h1>
        <p><?= htmlspecialchars($category['moTaLoaiSP'] ?? 'Khám phá các sản phẩm chất lượng cao') ?></p>
    </div>
</div>
...
<section class="products-section">
    <div class="container">
        <div class="products-header">
            <div class="products-count">Hiển thị <strong><?= count($products ?? []) ?></strong> sản phẩm</div>
        ...
        <div class="products-grid" id="productsGrid">
            <?php foreach ($products as $p): 
                ...
                $giaGoc = $p['giaXuat'] ?? 0;
                $giaSauGiam = $hasDiscount ? round($giaGoc * (100 - $discountPercent) / 100) : $giaGoc;
            ?>
            <div class="product-card" data-price="<?= $giaSauGiam ?>" data-name="<?= htmlspecialchars($p['tensp']) ?>">
                <?php if ($hasDiscount): ?>
                ...
                <div class="product-info">
                    ...
                    <div class="price-wrapper">
                        <?php if ($hasDiscount): ?>
                        <span class="current-price"><?= number_format($giaSauGiam) ?> ₫</span>
                        <span class="original-price" style="text-decoration: line-through; color: #999; font-size: 13px; margin-left: 8px;"><?= number_format($giaGoc) ?> ₫</span>
                        <?php else: ?>
                        <span class="current-price"><?= number_format($giaGoc) ?> ₫</span>
                        <?php endif; ?>
                    </div>
